# 08 — TP mini-app Python/Tkinter sur **macOS** : Claude Code (PAYANT, mode agent)
...
2. Ouvre-la dans Safari/Chrome, connecte-toi à ton compte Anthropic.
...
Persistant via `~/.zshrc`.

### Bonus — stocker dans le trousseau macOS
```bash
security add-generic-password -a "$USER" -s anthropic -w "sk-ant-..."
# Récupération dans .zshrc :
export ANTHROPIC_API_KEY=$(security find-generic-password -a "$USER" -s anthropic -w)
```
...
|----------|-------|-----------|
| `claude: command not found` | npm pas dans le PATH | `source ~/.zshrc` ou ferme/rouvre le terminal |
