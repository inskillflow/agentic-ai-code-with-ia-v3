# 03 — OpenRouter (gratuit) sur **macOS** : OpenClaude avec modèles cloud gratuits
...
> **Avantage clé** : pas besoin de RAM unifiée énorme, et modèles plus puissants que ce que tu peux faire tourner localement.
...
- Tu obtiens des modèles **plus puissants** que ce que ta RAM unifiée peut charger localement.
- Pas de pression sur ton GPU intégré.
...
2. Clique **Create Key**.
3. Donne un nom (ex. `openclaude-mac`).
...
Stocke-la dans un endroit sûr. Le **trousseau d'accès** macOS est une bonne option si tu veux la sauvegarder durablement.
...
Le shell par défaut macOS est **zsh**.
...
### Stocker dans le trousseau macOS (avancé)
```bash
...
| GPU | Apple Silicon recommandé | inutile |
| Confidentialité | totale (rien ne sort) | tout passe par OpenRouter |
| Taille de modèles | ~7-14 B sur 16 Go RAM unifiée | jusqu'à 405 B |
| Quotas | aucun | ~20 req/min, 200 req/jour |
| Latence | très basse (local) | moyenne (cloud) |
| Idéal pour | dev intensif, données sensibles | démos, étudiants sur Mac d'entrée de gamme |

> **Conseil pour Mac M1/M2 8 Go** : OpenRouter `:free` te donnera un meilleur résultat qu'Ollama avec un modèle Q4 minuscule.
